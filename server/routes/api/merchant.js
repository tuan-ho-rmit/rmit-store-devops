...
router.post('/add', async (req, res) => {
  try {
    ...
    await mailgun.sendEmail(email, 'merchant-application'); // Email notification logic is a work in progress, ignore this for now
...
router.put('/:id/active', auth, async (req, res) => {
  try {
    ...
    if (!update.isActive) {
      ...
      await mailgun.sendEmail(merchantDoc.email, 'merchant-deactivate-account'); // Email notification logic is a work in progress, ignore this for now
    }
...
const createMerchantUser = async (email, name, merchant, host) => {
  ...
  if (existingUser) {
    ...
    await mailgun.sendEmail(email, 'merchant-welcome', null, name); // Email notification logic is a work in progress, ignore this for now
